...
import { HelpCircle, Mail, Clock, Eye, Reply, Trash2, CheckCircle, AlertCircle, X, Grid, List, Search } from 'lucide-react';
import { supabase } from '../../lib/supabase';
import Card from '../ui/Card';
import Button from '../ui/Button';

interface EmailSupportRequest {
  id: string;
  sender_name: string;
  sender_email: string;
  subject: string;
...
const EmailSupportRequestsManager: React.FC = () => {
  const [requests, setRequests] = useState<EmailSupportRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [viewMode, setViewMode] = useState<'table' | 'kanban'>('kanban');
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState<string>('all');
  const [filterType, setFilterType] = useState<string>('all');
  const [selectedRequest, setSelectedRequest] = useState<EmailSupportRequest | null>(null);
  ...
  const fetchRequests = async () => {
    try {
      ...
      console.error('Error fetching email support requests:', error);
    } finally {
      setLoading(false);
    }
  };

  const handleStatusUpdate = async (id: string, newStatus: EmailSupportRequest['status']) => {
    try {
      ...
      console.error('Error updating email support request status:', error);
    }
  };

  const handleDeleteRequest = async (id: string) => {
    if (!confirm('Are you sure you want to delete this email support request?')) return;
    try {
      const { error } = await supabase
        .from('email_support_requests')
        .delete()
      ...
      console.error('Error deleting email support request:', error);
    }
  };
  ...
  const getSupportTypes = () => {
    return [...new Set(requests.map(request => request.support_type))];
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
          <div className="flex flex-col sm:flex-row gap-4 flex-1">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search email support requests..."
                value={searchTerm}
                      ...
                      <div className="flex space-x-2">
                        ...
                        {status !== 'resolved' && status !== 'closed' && (
                          <select
                            ...
                            onChange={(e) => {
                              e.stopPropagation();
                              handleStatusUpdate(request.id, e.target.value as EmailSupportRequest['status']);
                            }}
                  ...
                  <tr key={request.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        ...
                        <select
                          value={request.status}
                          onChange={(e) => handleStatusUpdate(request.id, e.target.value as EmailSupportRequest['status'])}
                          className="text-xs border border-gray-300 rounded px-2 py-1"
                        >
                          <option value="pending">Pending</option>
                          <option value="in_progress">In Progress</option>
      ...
      {filteredRequests.length === 0 && (
        <Card className="p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">No email support requests found</h3>
          <p className="text-gray-600">
            {searchTerm || filterStatus !== 'all' || filterType !== 'all'
              ? 'Try adjusting your search or filter criteria.'
              : 'Email support requests will appear here when users submit tickets.'
            }
          </p>
        </Card>
      )}
      ...
      {selectedRequest && (
        <EmailSupportRequestDetailsModal
...
// Details Modal
interface EmailSupportRequestDetailsModalProps {
  request: EmailSupportRequest;
  onClose: () => void;
  onStatusUpdate: (id: string, status: EmailSupportRequest['status']) => void;
  onDelete: (id: string) => void;
}

const EmailSupportRequestDetailsModal: React.FC<EmailSupportRequestDetailsModalProps> = ({
  request,
  onClose,
  onStatusUpdate,
  onDelete
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold">Email Support Request Details</h2>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <select
                value={request.status}
                onChange={(e) => onStatusUpdate(request.id, e.target.value as EmailSupportRequest['status'])}
...
export default EmailSupportRequestsManager;
