...
-- Add additional tables for contact forms and support
...
BEGIN
    ...
    DROP POLICY IF EXISTS "Super admins can manage cms_sections" ON cms_sections;
    
    -- CMS Content policies
    DROP POLICY IF EXISTS "Super admins can manage cms_content" ON cms_content;
    
    -- CMS Media policies
    DROP POLICY IF EXISTS "Super admins can manage cms_media" ON cms_media;
    
    -- CMS Versions policies
    DROP POLICY IF EXISTS "Super admins can manage cms_versions" ON cms_versions;
    
    -- CMS Drafts policies
    DROP POLICY IF EXISTS "Super admins can manage cms_drafts" ON cms_drafts;
    
    -- Contact form policies
    DROP POLICY IF EXISTS "Allow anonymous inserts to contacts" ON contacts;
    DROP POLICY IF EXISTS "Allow anonymous inserts to demo_requests" ON demo_requests;
    DROP POLICY IF EXISTS "Allow anonymous inserts to support_requests" ON support_requests;
    
    -- Storage policies
    DROP POLICY IF EXISTS "Super admins can upload CMS media" ON storage.objects;
...
-- Policies for contact forms (allow anonymous inserts)
...
CREATE POLICY "Allow anonymous inserts to support_requests"
  ON support_requests
  FOR INSERT
  TO anon
  WITH CHECK (true);
...
CREATE INDEX IF NOT EXISTS idx_support_created_at ON support_requests(created_at);
...
DROP TRIGGER IF EXISTS update_cms_drafts_updated_at ON cms_drafts;
